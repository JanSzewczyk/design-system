...
const INERT_STUB =
  ...
  // FORK (design-sync): CSF Next factory passthrough. This repo mixes CSF 3.0
  // (export default meta) with CSF Next (preview.meta()/meta.story()). The
  // latter call definePreview from @storybook/react-vite, which the inert stub
  // left undefined as a named export -> every CSF-Next story crashed at module
  // eval ("definePreview is not a function").
  //
  // compose() in the wrapper reads S.default for meta fields (args, component,
  // render). CSF Next has NO export default meta, so S.default=undefined and
  // compose() gets empty meta — meta-level args (like items:[...]) never reach
  // the story's render function. Fix: make meta.story() a closure that merges
  // meta.args/component/render into the returned story object, so story.args
  // already carries the merged values and compose() picks them up from st.args.
  "function __mkMeta(x){" +
  "  var meta=Object.assign({},x||{});" +
  "  meta.story=function(s){" +
  "    var st=Object.assign({},s||{});" +
  "    st.args=Object.assign({},meta.args,s&&s.args?s.args:{});" +
  "    if(!st.component&&meta.component)st.component=meta.component;" +
  "    if(!st.render&&meta.render)st.render=meta.render;" +
  "    if(meta.decorators&&meta.decorators.length){" +
  "      st.decorators=(meta.decorators).concat(s&&s.decorators?s.decorators:[]);" +
  "    }" +
  "    st.test=function(){return st;};" +
  "    return st;" +
  "  };" +
  "  return meta;" +
  "};" +
  "function __defPrev(p){var pv=Object.assign({},p||{});pv.meta=__mkMeta;return pv};" +
  "m.definePreview=__defPrev;m.__definePreview=__defPrev;" +
...
function exportedComponentFor(p, exported) {
  ...
  // FORK (design-sync): kebab-case files/dirs -> PascalCase exports. This repo
  // names files kebab (scroll-bar.tsx -> ScrollBar, sidebar/index.tsx ->
  // Sidebar) while exports are Pascal. Without this, a story importing a
  // subcomponent via a relative kebab path ('./scroll-bar', '.') falls through
  // to source bundling and gets a 2nd React context, so a global-shimmed
  // provider (from '~/components') and the source consumer disagree:
  // "X must be used within Provider". Mapping kebab->Pascal routes both to the
  // shipped global, unifying context.
  const pascal = (s) =>
    s
      .split("-")
  ...
  const dp = pascal(dir);
  if ((file === "index" || fp === dp) && exported.has(dp)) return dp;
  return null;
}
