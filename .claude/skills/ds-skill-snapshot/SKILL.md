...
exist in `@szum-tech/design-system`.
...
`@szum-tech/design-system` component or utility that replaces it. Load
...
Read the consumer's `package.json`. Compare its `@szum-tech/design-system`
...
- **Newer minor or major** → warn the user; before using any component
  listed in `_index.md`, grep `node_modules/@szum-tech/design-system` to
